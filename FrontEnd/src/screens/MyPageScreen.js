      ...
      <View style={styles.chipRow}>
        {activities.map((activity) => (
          <Chip
            key={activity}
            label={activity}
            active={form.activityLevel === activity}
            onPress={() => set("activityLevel", activity)}
            icon="run"
          />
        ))}
      </View>
...
export default function MyPageScreen() {
  ...
  const hasTargetNutrition = Boolean(dri);
