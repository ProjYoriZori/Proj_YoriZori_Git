...
const RAW_BASE_URL = (process.env.EXPO_PUBLIC_API_BASE_URL || defaultBaseUrl()).replace(/\/$/, '');
...
export function normalizeRecipe(raw) {
  ...
  const ingredients = Array.isArray(raw.ingredients)
    ? raw.ingredients.map((item) => ({
      ...
      amount: item.amount || item.amount_text || item.amountText || [item.quantity, item.unit].filter(Boolean).join(' '),
    }))
    : parsePublicRecipeIngredients(raw.RCP_PARTS_DTLS);

  const steps = Array.isArray(raw.steps)
    ? raw.steps.map((step, index) => ({
      id: step?.step_id || step?.stepId || step?.id || index + 1,
  ...
  return {
    ...
    method: raw.method || raw.cooking_method || raw.cookingMethod || raw.RCP_WAY2 || '',
    ...
    calories: Number(raw.calorie_kcal || raw.calories || nutrition.kcal || raw.INFO_ENG || 0),
    carbs: Number(raw.carbohydrate_g || raw.carbs || nutrition.carbs || raw.INFO_CAR || 0),
    protein: Number(raw.protein_g || raw.protein || nutrition.protein || raw.INFO_PRO || 0),
    fat: Number(raw.fat_g || raw.fat || nutrition.fat || raw.INFO_FAT || 0),
    sodium: Number(raw.sodium_mg || raw.sodium || nutrition.sodium || raw.INFO_NA || 0),
    imageUrl: raw.image_url || raw.imageUrl || raw.image || raw.main_image_url || raw.ATT_FILE_NO_MAIN || raw.ATT_FILE_NO_MK || '',
    ingredients,
    steps,
...
export function normalizeNutritionLog(raw) {
  ...
  return {
    ...
    recipeId: raw.recipe_id || raw.recipeId,
    date: raw.log_date || raw.logDate || raw.mealDate || raw.date || dateKey(new Date()),
    mealType: raw.meal_type || raw.mealType || raw.mealTime || 'SNACK',
    servingCount: Number(raw.serving_count || raw.servingCount || raw.multiplier || 1),
...
export function normalizeProfile(raw) {
  ...
  return {
    ...
    height: raw.height_cm ? Number(raw.height_cm) : raw.heightCm ? Number(raw.heightCm) : Number(raw.height || 0) || '',
    weight: raw.weight_kg ? Number(raw.weight_kg) : raw.weightKg ? Number(raw.weightKg) : Number(raw.weight || 0) || '',
...
function toPantryRequest(body = {}) {
  return {
    ...
    quantityText: [body.quantity, body.unit].filter(Boolean).join(' ') || body.quantityText || null,
...
function toNutritionLogRequest(body = {}) {
  return {
    ...
    customFoodName: body.customFoodName || body.food_name || body.foodName || null,
    mealDate: body.mealDate || body.log_date || body.logDate || body.date || dateKey(new Date()),
    mealTime: body.mealTime || body.meal_type || body.mealType || 'SNACK',
    multiplier: Number(body.multiplier || body.serving_count || body.servingCount || 1),
    nutrition: {
...
async function readRequiredList(path, normalizer) {
  try {
    const payload = await request(path);
    return { data: unwrapList(payload).map(normalizer), online: true, error: null };
  } catch (error) {
...
export async function loadInitialData() {
  const [recipeResult, pantryResult, shoppingResult, profileResult, logsResult, customFoodsResult] = await Promise.all([
    ...
    readOne(`${ENDPOINTS.nutritionDailySummary}?date=${dateKey(new Date())}`, (payload) => unwrapList(payload.meals).map(normalizeNutritionLog), () => []),
  ...
  return {
    ...
    backendOnline: [recipeResult, pantryResult, shoppingResult, profileResult, logsResult, customFoodsResult].some((result) => result.online),
  };
}
...
export const api = {
  ...
  getRecipes: ({ query = '', ingredient = '', limit = 50, page = 0, sort = 'latest' } = {}) => {
    ...
    return request(`${ENDPOINTS.recipes}?${params.toString()}`).then((payload) => unwrapList(payload).map(normalizeRecipe));
  },
  ...
  createPantryItem: (body) => request(ENDPOINTS.pantryItems, { method: 'POST', body: toPantryRequest(body) }).then(normalizePantryItem),
  updatePantryItem: (id, body) => request(`${ENDPOINTS.pantryItems}/${id}`, { method: 'PATCH', body: toPantryRequest(body) }).then(normalizePantryItem),
  deletePantryItem: (id) => request(`${ENDPOINTS.pantryItems}/${id}`, { method: 'DELETE' }),
  createShoppingItem: (body) => request(ENDPOINTS.shoppingItems, { method: 'POST', body: { name: body.name || body.item_name || body.itemName } }).then((payload) => unwrapList(payload).map(normalizeShoppingItem)),
  updateShoppingItem: (id, body) => request(`${ENDPOINTS.shoppingItems}/${id}`, { method: 'PATCH', body: toShoppingPatchRequest(body) }).then(normalizeShoppingItem),
  generateShoppingItems: (body) => request(ENDPOINTS.shoppingGenerate, { method: 'POST', body: { recipeId: body.recipeId || body.recipe_id } }).then((payload) => unwrapList(payload).map(normalizeShoppingItem)),
  createNutritionLog: (body) => request(ENDPOINTS.nutritionLogs, { method: 'POST', body: toNutritionLogRequest(body) }).then(normalizeNutritionLog),
  saveProfile: (profile) => request(ENDPOINTS.me, { method: 'PATCH', body: toProfileRequest(profile) }).then(normalizeProfile),
  addAvoidIngredient: (body) => request(ENDPOINTS.avoidIngredients, { method: 'POST', body }),
  removeAvoidIngredient: (id) => request(`${ENDPOINTS.avoidIngredients}/${id}`, { method: 'DELETE' }),
  addFavorite: (body) => request(ENDPOINTS.favorites, { method: 'POST', body: toFavoriteRequest(body) }),
  removeFavorite: (body) => request(ENDPOINTS.favorites, { method: 'DELETE', body: toFavoriteRequest(body) }),
  ...
  createCustomFood: (body) => request(ENDPOINTS.customFoods, { method: 'POST', body: toCustomFoodRequest(body) }).then(normalizeCustomFood),
};
