      ...
      addShoppingItem: async (input) => {
        ...
        try {
          const created = await api.createShoppingItem(
            shoppingPayload(optimistic),
          );
          if (created.length) {
            setShoppingItems((items) =>
              items.map((item) =>
                item.id === optimistic.id ? created[0] : item,
              ),
            );
          }
        } catch {
        ...
        try {
          ...
          if (created.length) {
            setShoppingItems((items) => {
              const next = [...items];
              const seen = new Set(
                items.map((item) => normalizeName(item.name)),
              );
      ...
      addNutritionLogFromRecipe: async (recipe, mealType = "점심") => {
        ...
        try {
          ...
          setNutritionLogs((logs) =>
            logs.map((item) => (item.id === log.id ? created : item)),
          );
        } catch {
          setBackendOnline(false);
        }
      ...
      addCustomFood: async (input) => {
        ...
        try {
          ...
          setCustomFoods((foods) =>
            foods.map((item) => (item.id === food.id ? created : item)),
          );
        } catch {
          setBackendOnline(false);
        }
